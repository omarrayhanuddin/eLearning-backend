...
from .tasks import send_new_material_notification
...
class ModuleSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        emails = course.enrollments.values_list("student__email", flat=True)
        if not emails:
            return data
        send_new_material_notification.delay(
            student_emails=list(emails),
...
class ModuleContentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        data = super().create(validated_data)
        emails = data.module.course.enrollments.values_list("student__email", flat=True)
        if not emails:
            return data
        send_new_material_notification.delay(
            student_emails=list(emails), 
            course_name=data.module.course.title,
            material_title=data.title, 
            is_module=True
        )
